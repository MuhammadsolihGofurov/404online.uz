...
function FourthPage({ info, user, loading }) {
  ...
  const handleDeleteQuestion = async (question) => {
    if (
      !window.confirm("Delete this question? This action cannot be undone.")
    ) {
      return;
    }
    ...
    if (!data) {
      toast.error("Unable to delete question right now");
      return;
    }
    ...
    // Optimistic update removes the card instantly
    await mutate(updatedData, false);

    try {
      await authAxios.delete(`/mock-questions/${questionId}/`);
      toast.success("Question deleted");
      mutate(); // optional revalidation for safety
    } catch (error) {
      if (error?.response?.status === 404) {
        toast.success("Question deleted (already removed)");
        return;
      }
              ...
              <div className="p-5 bg-white border rounded-3xl border-slate-200">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                      type="button"
                      disabled={!canFinish || !mockId}
                      onClick={() =>
                        router.push(`${MOCKS_VIEW_URL}?mock_id=${mockId}`)
                      }
