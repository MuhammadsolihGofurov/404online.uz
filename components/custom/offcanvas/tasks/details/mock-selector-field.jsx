...
import { Search, Clock, FileText, CheckCircle2 } from "lucide-react"; // Lucide piktogrammalari

const MockSelectorField = ({ title, type, value, onChange, locale }) => {
  const intl = useIntl();
  ...
  const getKey = (pageIndex, previousPageData) => {
    ...
    return `/mocks/${type}/?page=${
      pageIndex + 1
    }&page_size=10&status=EXAM&search=${debouncedSearch}`;
  };
  ...
  const mocks = data ? data.flatMap((page) => page.results) : [];
  const isLoadingMore =
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {/* Sarlavha va uning ostida Search */}
      <div className="flex flex-col gap-2">
        <h3 className="text-left font-semibold text-gray-800 text-sm sm:text-base">
          {title}
        </h3>
      ...
      <div className="flex flex-col gap-2 h-[350px] overflow-y-auto p-2 border border-gray-100 rounded-xl bg-gray-50/50 scrollbar-thin">
        {mocks.length === 0 && !isValidating ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
            <FileText size={32} strokeWidth={1.5} />
            <span className="text-xs">
              {intl.formatMessage({ id: "No mocks found" })}
            </span>
          </div>
        ) : (
          mocks.map((mock) => (
            ...
            >
              <div className="flex justify-between items-start">
                <input
                  ...
                  name={type}
                  value={mock.id}
                  checked={value === mock.id}
                  onChange={() => onChange(mock.id)}
                  className="hidden"
                />
                <span
                  className={`font-semibold text-sm transition-colors ${
                    value === mock.id ? "text-main" : "text-gray-700"
                  }`}
                >
                  {mock.title}
                </span>
        ...
        <div ref={ref} className="py-4 flex justify-center min-h-[50px]">
          {isLoadingMore ? (
            ...
            <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">
              {intl.formatMessage({ id: "All loaded" })}
            </span>
          ) : null}
        </div>
      </div>
...
export default MockSelectorField;
