...
export default function TasksHomeworkLists({ role, loading }) {
  ...
  const homeworkId = findParams("homework_id") || "";

  const { data: datas, isLoading } = useSWR(
    [
      "/submissions/homework-review-list",
      router.locale,
      homeworkId,
      currentPage,
      offcanvasClosed,
    ...
    ([url, locale, hId, page]) =>
      fetcher(
        `${url}/?homework_task_id=${hId}&page=${page}&page_size=12`,
        {
          headers: {
